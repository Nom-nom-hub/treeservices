...
export default function LightTheme() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-stone-50 via-white to-green-50/30">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[200] focus:bg-green-600 focus:text-white focus:px-4 focus:py-2 focus:rounded-lg">Skip to content</a>

      <nav className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-green-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-8">
            {['Services', 'Service Areas', 'Portfolio'].map(item => (<a key={item} href={`#${item.toLowerCase().replace(/\s+/g, '-').replace('service-areas', 'areas')}`} className="text-stone-600 hover:text-green-700 font-medium text-sm transition-colors">{item}</a>))}
            <a href="tel:2399440073" className="bg-green-600 hover:bg-green-500 text-white px-6 py-2.5 rounded-full font-semibold transition-all active:scale-95 shadow-md hover:shadow-lg flex items-center gap-2"><Phone className="w-4 h-4" /> (239) 944-0073</a>
          </div>
        ...
        <div className={`md:hidden fixed inset-0 top-20 bg-white/98 z-40 transition-all duration-300 ${mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
          <div className="flex flex-col items-center justify-center h-full gap-4 -mt-20">
            {[{ label: 'Services', href: '#services' },{ label: 'Service Areas', href: '#areas' },{ label: 'Portfolio', href: '#portfolio' },{ label: 'Contact', href: '#contact' }].map(l => (<a key={l.href} href={l.href} onClick={closeMobileMenu} className="text-2xl text-stone-700 hover:text-green-600 font-semibold transition-colors py-2">{l.label}</a>))}
            <a href="tel:2399440073" onClick={closeMobileMenu} className="mt-6 bg-green-600 text-white px-10 py-4 rounded-full font-bold text-lg active:scale-95 flex items-center gap-3"><Phone className="w-5 h-5" /> (239) 944-0073</a>
          </div>
        </div>
      </nav>
      ...
      <main id="main-content" aria-hidden={!!lightboxImage}>
        <section className="relative pt-28 md:pt-32 pb-20 px-4 bg-gradient-to-b from-green-50 via-white to-stone-50 overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto relative">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div className="reveal">
                <div className="inline-block bg-green-100 text-green-800 px-4 py-2 rounded-full text-sm font-semibold mb-6 border border-green-300"><Check className="w-4 h-4 inline mr-1" /> 33+ Years of Excellence</div>
                <h1 className="text-5xl md:text-6xl font-bold text-stone-900 mb-6 leading-tight">Your Trees Deserve <span className="text-green-600">Expert Care</span></h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <a href="tel:2399440073" className="bg-green-600 hover:bg-green-500 text-white font-bold py-4 px-8 rounded-xl text-center transition-all active:scale-95 flex items-center justify-center gap-2 shadow-lg hover:shadow-green-600/30"><AlertCircle className="w-5 h-5" /> Emergency Service Now</a>
                  <a href="#contact" className="border-2 border-stone-300 text-stone-700 hover:border-green-500 hover:text-green-700 font-bold py-4 px-8 rounded-xl text-center transition-all active:scale-95">Free Estimate</a>
                </div>
                <div className="mt-12 grid grid-cols-3 gap-6">
                  {[{ target: '33+', label: 'Years Experience' },{ target: '62', label: '5-Star Reviews' },{ target: '24/7', label: 'Emergency Ready' }].map((s, i) => (
                    <div key={i} className="border-l-2 border-green-500 pl-4 reveal" style={{ transitionDelay: `${i * 0.1}s` }}>
                      <p className="text-3xl font-bold text-green-600"><AnimatedCounter target={s.target} /></p>
        ...
        <div className="border-y border-green-200 bg-green-50 py-3 overflow-hidden">
          <div className="flex animate-[marquee_25s_linear_infinite] whitespace-nowrap">
            {[...trustSignals, ...trustSignals].map((s, i) => (<span key={i} className="inline-flex items-center gap-2 mx-6 text-sm text-green-700/60 font-medium"><Trophy className="w-3.5 h-3.5 text-green-500/40" />{s}</span>))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-6">
            {serviceData.map((s, idx) => { const Icon = serviceIcons[idx];
              return (<div key={idx} className="reveal group relative bg-stone-50 rounded-2xl p-[1px] hover:shadow-xl transition-all duration-300 hover:-translate-y-2" style={{ transitionDelay: `${idx * 0.1}s` }}>
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-green-500/0 via-green-400/15 to-green-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 animate-[shimmer_2s_linear_infinite] bg-[length:200%_100%]" />
        ...
        <section id="areas" className="py-20 px-4 bg-stone-50">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {serviceAreas.map((area, idx) => (<div key={idx} className="reveal group relative bg-white rounded-2xl p-[1px] hover:shadow-lg transition-all duration-300" style={{ transitionDelay: `${idx * 0.15}s` }}>
              <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-green-500/0 via-green-400/10 to-green-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 animate-[shimmer_2s_linear_infinite] bg-[length:200%_100%]" />
              <div className="relative bg-white rounded-2xl p-8 h-full border border-stone-200 group-hover:border-green-200 transition-colors">
                <div className="flex items-center gap-3 mb-6"><MapPin className="w-6 h-6 text-green-600 flex-shrink-0" /><h3 className="text-2xl font-bold text-stone-800">{area.county}</h3></div>
        ...
        <section id="portfolio" className="py-20 px-4 bg-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {portfolioImages.map((img, idx) => (<div key={idx} onClick={() => setLightboxImage(img.src)} className="reveal group relative bg-stone-100 rounded-2xl overflow-hidden hover:shadow-lg transition-all cursor-pointer aspect-square border border-stone-200 hover:border-green-300" style={{ transitionDelay: `${idx * 0.1}s` }} role="button" tabIndex={0} aria-label={`View: ${img.alt}`} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setLightboxImage(img.src); } }}>
              ...
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center"><div className="opacity-0 group-hover:opacity-100 transition-all"><div className="w-12 h-12 rounded-full bg-green-600/90 flex items-center justify-center shadow-lg"><svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" /></svg></div></div></div>
            </div>))}
          </div></div>
        </section>

        <section id="contact" className="py-20 px-4 bg-gradient-to-b from-stone-50 to-green-50/50">
      ...
      <footer className="bg-stone-900 text-stone-400 py-12 px-4">
        <div className="max-w-7xl mx-auto"><div className="grid md:grid-cols-4 gap-8 mb-8">
          <div className="reveal"><Logo className="h-16 w-auto mb-4 brightness-0 invert" /><p className="text-sm leading-relaxed">Professional tree care serving Southwest Florida since 1991. Licensed &amp; insured.</p></div>
      ...
      <a href="tel:2399440073" className={`md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-green-600 hover:bg-green-500 text-white font-bold py-3.5 px-6 rounded-full flex items-center gap-2.5 shadow-lg transition-all active:scale-95 ${showFloatingCall ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}><span className="relative flex h-2.5 w-2.5"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75" /><span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-white" /></span><Phone className="w-4 h-4" /> (239) 944-0073</a>
      <button onClick={scrollToTop} className={`fixed bottom-8 right-8 z-50 w-12 h-12 bg-green-600 hover:bg-green-500 text-white rounded-full flex items-center justify-center shadow-lg transition-all active:scale-90 ${showBackToTop ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}><ArrowUp className="w-5 h-5" /></button>
